...
export default function CallSummary({
  ...
  // Extract Fiji Hindi part from "FijiHindi - English" format
  const extractFijiHindi = (phrase: string): string => {
    const parts = phrase.split(" - ");
    return parts[0].trim();
  };

  const playPhrase = useCallback((phrase: string, index: number) => {
    const fijiHindi = extractFijiHindi(phrase);
    setPlayingIndex(index);
    speakFijiHindi(fijiHindi, {
        ...
        <div className="space-y-3">
          {phrasesLearned.map((phrase, index) => (
            ...
            >
              ...
              <p className="flex-1 text-charcoal dark:text-white">{phrase}</p>
              {/* Audio button */}
              <button
                onClick={() => playPhrase(phrase, index)}
